...
module cMin(iSize,iPared,intcH)
{
    ...
    difference()
    {
        ...
        cube([iSize*.8,iPared*2.2,intcH+1]);
    }
    }
}
...
module oMin(intoSize, intoH)
{
  difference()
  {
    cylinder( r=(intoSize*0.6), intoH, $fn=100 );
    
    translate([0,0,-0.1])
    cylinder( r=(intoSize*0.3), intoH*2, $fn=100 );
...
module eMin(iSize,iPared,inteH)
{
    rotate([0,0,10])
    ...
    {
        translate()
        cMin(iSize, iPared*1.8, inteH);
        
        translate([0,-iPared*1.3,-0.1])
        cube([iSize*.8,iPared,inteH+1]);
    }
    
    rotate([0,0,15])
    translate([-iSize*.15,iPared*4,0])
    cube([iSize*.9,iPared*3.5,inteH]);
    }
}
...
module logoSimple(letsize,letpared,leth,letoffset)
{
    ...
    eMin(letsize*.53,letpared*.3,leth);
    
    translate([(letsize*2.8)+(4*letoffset),0,0])
    cMin(letsize*.53,letpared*.53,leth);
...
module logo(letsize,letpared,leth,letoffset)
{
    ...
    eMin(letsize*.5,letpared*.3,leth);
